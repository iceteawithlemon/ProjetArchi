...
int gen_need_regids()
{
    return ((icode) == (I_RRMOVL)||(icode) == (I_ALU)||(icode) == (I_ALUI)||(icode) == (I_PUSHL)||(icode) == (I_POPL)||(icode) == (I_IRMOVL)||(icode) == (I_RMMOVL)||(icode) == (I_MRMOVL)||(icode) == (I_JREG)||(icode) == (I_JMEM));
}

int gen_need_valC()
{
    return ((icode) == (I_IRMOVL)||(icode) == (I_RMMOVL)||(icode) == (I_MRMOVL)||(icode) == (I_JXX)||(icode) == (I_CALL)||(icode) == (I_ALUI)||(icode) == (I_JMEM));
}

int gen_instr_valid()
{
    return ((icode) == (I_NOP)||(icode) == (I_HALT)||(icode) == (I_RRMOVL)||(icode) == (I_IRMOVL)||(icode) == (I_RMMOVL)||(icode) == (I_MRMOVL)||(icode) == (I_ALU)||(icode) == (I_ALUI)||(icode) == (I_JXX)||(icode) == (I_CALL)||(icode) == (I_RET)||(icode) == (I_PUSHL)||(icode) == (I_POPL)||(icode) == (I_JREG)||(icode) == (I_JMEM)||(icode) == (I_LEAVE));
}
...
int gen_srcB()
{
    return (((icode) == (I_ALU)||(icode) == (I_ALUI)||(icode) == (I_RMMOVL)||(icode) == (I_MRMOVL)||(icode) == (I_JMEM)) ? (rb) : (((icode) == (I_IRMOVL)) & ((ifun) == 1)) ? (rb) : ((icode) == (I_PUSHL)||(icode) == (I_POPL)||(icode) == (I_CALL)||(icode) == (I_RET)) ? (REG_ESP) : ((icode) == (I_LEAVE)) ? (REG_EBP) : 1 ? (REG_NONE) : 0);
}
...
int gen_aluA()
{
    return (((icode) == (I_RRMOVL)||(icode) == (I_ALU)) ? (vala) : ((icode) == (I_IRMOVL)||(icode) == (I_RMMOVL)||(icode) == (I_MRMOVL)||(icode) == (I_ALUI)||(icode) == (I_JMEM)) ? (valc) : ((icode) == (I_CALL)||(icode) == (I_PUSHL)) ? -4 : ((icode) == (I_RET)||(icode) == (I_POPL)||(icode) == (I_LEAVE)) ? 4 : 0);
}
